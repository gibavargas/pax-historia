const FALLBACK_COUNTRY_CODES = [
  ...
  "VN:VNM", "YE:YEM", "ZM:ZMB", "ZW:ZWE", "XK:XXK",
];

export const getFallbackCountries = (locale) => {
  const displayNames =
    typeof Intl !== "undefined" && Intl.DisplayNames
      ? new Intl.DisplayNames(locale ? [locale] : undefined, { type: "region" })
      : null;

  return FALLBACK_COUNTRY_CODES.map((entry) => {
    const [regionCode, code] = entry.split(":");
    const name = displayNames?.of(regionCode) || code;
    return { code, name };
